...
Source0:       https://github.com/lkiesow/opencast/archive/%{srcversion}.tar.gz
...
install -p -D -m 0755 \
   %{buildroot}%{_datadir}/opencast/docs/scripts/service/etc-init.d-opencast \
   %{buildroot}%{_initrddir}/opencast

# Patch up some directories

# Systemd unit file (path to binary)
sed -i 's#/opt/#/usr/share/#' %{buildroot}%{_initrddir}/opencast
...
%post
/sbin/chkconfig --add opencast || :


%preun
if [ $1 -eq 0 ]; then
   /sbin/service opencast stop >/dev/null 2>&1 || :
   /sbin/chkconfig --del opencast || :
fi
...
%changelog
* Mon Jan 25 2016 Lars Kiesow <lkiesow@uos.de> - 2.1.1-1
