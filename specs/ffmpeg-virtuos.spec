...
%build
./configure --prefix=/opt/ffmpeg%{_prefix} --libdir=/opt/ffmpeg%{_libdir} \
            --shlibdir=/opt/ffmpeg%{_libdir} --mandir=/opt/ffmpeg%{_mandir} \
   --disable-shared \
   --enable-static \
...
rm -f doc/Makefile
rm -f %{buildroot}/opt/ffmpeg/usr/share/doc/ffmpeg/*.html
rm -rf %{buildroot}/opt/ffmpeg%{_mandir}/man3/
rm -f %{buildroot}/opt/ffmpeg%{_libdir}/*.a
...
%defattr(-,root,root,-)
/opt/ffmpeg%{_bindir}/*
/opt/ffmpeg%{_datadir}/ffmpeg
/opt/ffmpeg%{_mandir}/man1/*


%files devel
%defattr(-,root,root,-)
/opt/ffmpeg%{_includedir}/*
/opt/ffmpeg%{_libdir}/pkgconfig/*.pc
...
- Fised libvpx depedency

* Sun Jan 19 2014 Lars Kiesow <lkiesow@uos.de> - 2.1.3-2
- Build against new libvpx version (vp9 support)
